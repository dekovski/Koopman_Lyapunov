...
writer.setup(fig,"Alpha_set2.gif")
tic = time.time()
alpha_history = []
for k in range(100):
    print(k)
    # Do some stuff to ake cool animations
    alpha_history.append(intersection.chebXc)
    """
    ax = intersection.project([1,4]).plot(ax, color = (k/99)*np.ones(3), alpha = 1.0, linestyle = '-', linewidth = 0.5, edgecolor = "red")
    ...
    V_ = V_basis(X,P)
    A_ub = (Delta_V + beta*V_).T
    b = beta*gamma*np.ones(A_ub.shape[0])
    idx = np.argwhere(np.matmul(V_.T,alpha.T).T[0]<b).T[0]
    if(len(idx)==0):
        gamma = gamma*5
        continue
    A_ub = A_ub[idx,:]
    ...
    # If this intersection is empty, try increasing gamma, hope it works.
    while(pc.is_empty(intersection_)):
        gamma = gamma + 1e-5 # Or should be increase beta instead?
        b = beta*gamma*np.ones(A_ub.shape[0])
        p1 = pc.Polytope(A_ub,b)
        intersection_ = p1.intersect(intersection)
        #print("terminating")
        #break
    intersection = intersection_
    print("Volume of alpha set = " + str(pc.volume(intersection)))
toc = time.time()
writer.finish()
...
fig, ax0 = plt.subplots()
writer =  PillowWriter(10)
writer.setup(fig,"Level_set.gif")
...
for i in range(100):
    ...
    if(i==0):     
        fig.colorbar(im, ax=ax0)
    con = ax0.contour(X_,Y_, Z, [np.log10(gamma)], linewidths=1, colors = 'k')
    con.cmap
    ax0.text(1.5, 3.2, "Iteration: " + str(i), fontsize=12, color='r'); 
    writer.grab_frame()
    ax0.clear()
writer.finish()   
...
X,Y,Start = sample_data(basis_fun,1,1,eps=eps) 
V_ = V_basis(X,P)
res = linprog(V_[:,0], A_ub=intersection.A,b_ub=intersection.b,bounds = bounds,method='revised simplex')
alpha1 = res.x
alpha2 = intersection.chebXc
alpha = alpha1 + 0.1*(alpha2-alpha1)
alpha = np.reshape(alpha,(1,-1))
...
print("Positive solution means success")

""" Solve using sympy """
### DIDN"T SOLVE,RETURNS EMPTY 
"""
lam = sympy.symbols('lambda', real = True)
L = Candidate_dot - lam* (Candidate)
gradL = [sympy.diff(L,c) for c in [x,y]] # gradient of Lagrangian w.r.t. (x,y)
KKT_eqs = gradL + [Candidate]
KKT_eqs
stationary_points = sympy.solve(KKT_eqs, [x, y, lam], dict=True) # solve the KKT equations
stationary_points 
"""
...
"""
TODO :
1) Prove or disprove that for a given invariant region R, there is a lyapunov like function 
   that satisfies \dot{V} = \beta(\gamma - V) whenever V(x)\le\gamma, 
   and {x|v(x)\le \gamma} \subset R.
2) Impose constraint that 
"""
